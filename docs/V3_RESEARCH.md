...
features, and existing semantic predictors; it does not use MusicFM. Prior ratings
...
This log records positive and negative evidence so a development improvement
cannot be presented as a production-quality jump. The complete immutable
artifacts, implementation, and commits referenced below are preserved on branch
`exp/v3-musicfm-canary`; this document is copied to `main` as the public research
record.
