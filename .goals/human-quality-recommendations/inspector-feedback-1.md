...
4. **CRITICAL: prohibited commit-metadata violation** — Commit message contains a prohibited attribution trailer which user explicitly rejects.
...
### 4. **CRITICAL: Commit Metadata Violation**

**Commit Message (4c71e4e):**
```
[prohibited attribution trailer removed]
```

**User Requirement:**
> "The user explicitly rejects any prohibited attribution/tagline. Flag every attribution trailers in commits or new goal/product docs as requiring removal."
...
**Required Fix:**
- Amend commit to remove prohibited trailer
...
1. **Remove prohibited attribution from commit**
   - Rewrite the final squash without a prohibited attribution trailer
...
5. **Prohibited attribution** violates user requirement
...
Execute the production evaluation pipeline (baseline freeze → enhanced eval → compare), fix the manual-pairs leakage, remove prohibited attribution, and run the held-out coherence test. This will either show the improvements are real (PASS) or reveal they do not hold on production data (revert and try different approaches).
