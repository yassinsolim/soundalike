...
function validExport() {
  return sign({
    schema_version: 2,
    submission_schema: "fulltrack_jamendo_listener_submission_v2",
    source_kind: "human_listener",
    provider: "hosted_private_fulltrack_v2_evaluator",
    anonymous_rater_id: `anon-v2-${"1".repeat(24)}`,
    session_id: `fulltrack-session-${"2".repeat(24)}`,
    protocol_sha256: V2_PROTOCOL_SHA256,
    pilot_pack_sha256: V2_PILOT_PACK_SHA256,
...
async function submit(
  ...
  const wrapper =
    options.wrapper === undefined
      ? { consent: true, study: "fulltrack-v2-pilot", ratings }
      : options.wrapper;
  await createV2Handler(storage, options.deploymentHost)(
...
test("accepts v2 evidence only in the distinct immutable private prefix", async () => {
  ...
  assert.equal(
    put.pathname,
    `${V2_BLOB_PREFIX}fulltrack-session-${"2".repeat(24)}/${res.body.receipt_sha256}.json`,
  );
  assert.equal(put.pathname.startsWith("human-ratings/v17/"), false);
  ...
  assert.equal(parseV2StoredRecordBytes(put.body, put.pathname).digest, res.body.receipt_sha256);
...
test("requires exact wrapper consent, study and at least one rating", async () => {
  for (const wrapper of [
    { consent: false, study: "fulltrack-v2-pilot", ratings: validExport() },
    { consent: true, study: "v17", ratings: validExport() },
    {
      consent: true,
      study: "fulltrack-v2-pilot",
      ratings: validExport(),
      extra: true,
    },
  ]) {
...
test("enforces bounded strict JSON and POST-only private ingestion", async () => {
  let result = await submit(validExport(), new MemoryStorage(), {
    contentLength: MAX_V2_BODY_BYTES + 1,
  });
  ...
  const rawBody =
    `{"consent":true,"consent":true,"study":"fulltrack-v2-pilot",` +
...
test("rejects untrusted origins and never exposes list or read behavior", async () => {
  ...
  const source = readFileSync(
    new URL("../api/ratings-v2.js", import.meta.url),
    "utf8",
  );
...
test("analyst-only tool lists the v2 prefix and validates downloads", async () => {
  ...
  const directory = await mkdtemp(join(tmpdir(), "soundalike-v2-inbox-"));
  try {
    const first = await downloadV2(directory, analystStorage);
    const second = await downloadV2(directory, analystStorage);
    assert.deepEqual(first, { downloaded: 1, existing: 0 });
    assert.deepEqual(second, { downloaded: 0, existing: 1 });
    assert.equal(calls.every((call) => call.prefix === V2_BLOB_PREFIX), true);
