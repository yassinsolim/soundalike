...
import {
  analyzeV6Snapshots,
  signFlipInference,
} from "../tools/ratings-v6-analysis.js";
...
test("scores all six method predictions and repeated-anchor consistency", () => {
  const older = {
    session_id: "v6-session-111111111111111111111111",
  ...
  const report = analyzeV6Snapshots([older, latest], pack, privateMap);
...
test("counts skips without treating repeated anchors as independent evidence", () => {
  const record = {
    session_id: "v6-session-222222222222222222222222",
    exported_at: "2026-08-13T00:00:00.000Z",
    received_at: "2026-08-13T00:00:01.000Z",
  ...
  const report = analyzeV6Snapshots([record], pack, privateMap);
...
test("rejects tampered private method orders before scoring", () => {
  ...
  assert.throws(
    () => analyzeV6Snapshots([], pack, tampered),
...
test("uses whole tasks as inference clusters", () => {
  ...
  const report = analyzeV6Snapshots(
    [
      {
        session_id: "v6-session-333333333333333333333333",
        exported_at: "2026-08-13T00:00:00.000Z",
        received_at: "2026-08-13T00:00:01.000Z",
...
test("does not multiply task clusters when listeners rate the same task", () => {
  const records = ["4", "5"].map((digit, index) => ({
    session_id: `v6-session-${digit.repeat(24)}`,
  ...
  const report = analyzeV6Snapshots(records, pack, privateMap);
  const acoustic =
    report.method_pairwise_prediction_evidence.acoustic_control;
  const comparison =
    report.paired_method_inference.acoustic_control__vs__fixed_v4;
  ...
  assert.equal(acoustic.listener_sign_flip_over_chance_one_sided_p, 0.25);
  assert.equal(acoustic.listener_sign_flip_method, "exact_enumeration");
  assert.equal(acoustic.listener_sign_flip_draws, 4);
  ...
  assert.equal(comparison.listener_sign_flip_two_sided_p, 0.5);
  assert.equal(comparison.listener_sign_flip_method, "exact_enumeration");
  assert.equal(comparison.listener_sign_flip_draws, 4);
});

test("uses bounded deterministic Monte Carlo for many listener clusters", () => {
  const deltas = Array.from({ length: 30 }, (_, index) => ({
    numerator: (index % 2 ? -1 : 1) * (index + 1),
    denominator: 31,
  }));
  const first = signFlipInference(deltas, "two-sided");
  const second = signFlipInference(deltas.slice().reverse(), "two-sided");

  assert.deepEqual(first, second);
  assert.equal(first.method, "deterministic_monte_carlo");
  assert.equal(first.draws, 100_000);
  assert.equal(first.nonzero_clusters, 30);
  assert.equal(first.p > 0 && first.p <= 1, true);
});

test("exact rational sign flips are independent of cluster ordering", () => {
  const deltas = Array.from({ length: 16 }, (_, index) => ({
    numerator: (index % 2 ? -1 : 1) * (index + 1),
    denominator: 17,
  }));
  const forward = signFlipInference(deltas, "two-sided");
  const reversed = signFlipInference(deltas.slice().reverse(), "two-sided");

  assert.deepEqual(forward, reversed);
  assert.equal(forward.method, "exact_enumeration");
  assert.equal(forward.draws, 65_536);
});
