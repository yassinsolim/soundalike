...
const PATHNAME =
  /^human-ratings\/strict-v5-ranking-v1\/(v5-session-[a-f0-9]{24})\/([a-f0-9]{64})\.json$/;
...
async function readBounded(stream) {
  ...
  for await (const chunk of stream) {
    ...
    size += buffer.length;
    if (size > MAX_V5_STORED_BYTES) {
      throw new Error("A private V5 ratings object exceeds the expected size.");
    }
    chunks.push(buffer);
  }
...
async function destinationFor(root, sessionId, digest) {
  ...
  if (
    ...
    throw new Error("Unsafe private V5 inbox destination.");
  }
  await mkdir(directory, { recursive: true, mode: 0o700 });
  const metadata = await lstat(directory);
  if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
    throw new Error("Private V5 inbox session path is not a safe directory.");
  }
  return destination;
}
...
export async function downloadV5(
  ...
  do {
    ...
    if (
      ...
      throw new Error("Private V5 inbox listing returned an invalid page.");
    }
    for (const blob of page.blobs) {
      ...
      if (
        ...
        blob.size < 2 ||
        blob.size > MAX_V5_STORED_BYTES
      ) {
        throw new Error("Unexpected object in the private V5 inbox.");
      }
      ...
      if (
        ...
        throw new Error("A private V5 ratings object could not be downloaded.");
      }
      const bytes = await readBounded(result.stream);
      if (bytes.length !== blob.size) {
        throw new Error("A private V5 ratings object changed during download.");
      }
      parseV5StoredRecordBytes(bytes, blob.pathname);
      ...
      try {
        ...
        if (!local.equals(bytes)) {
          throw new Error("An existing V5 inbox file failed integrity comparison.");
        }
        existing += 1;
      }
    }
    if (page.hasMore && typeof page.cursor !== "string") {
      throw new Error("Private V5 inbox listing omitted its continuation cursor.");
    }
...
async function main() {
  ...
  if (!acknowledged || positional.length !== 1) {
    console.error(
      "Usage: npm run ratings:v5-inbox -- <output-dir> --acknowledge-private-data",
    );
    process.exitCode = 2;
    return;
  }
  try {
    const result = await downloadV5(positional[0]);
    console.log(
      `Private V5 inbox sync complete: ${result.downloaded} downloaded, ${result.existing} already present.`,
    );
  } catch {
    console.error(
      "Private V5 inbox download failed. Check authorized Blob credentials and retry.",
    );
    process.exitCode = 1;
  }
}
