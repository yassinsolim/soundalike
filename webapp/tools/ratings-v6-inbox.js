...
async function readBounded(stream) {
  ...
  for await (const chunk of stream) {
    ...
    size += buffer.length;
    if (size > MAX_V6_STORED_BYTES) {
      throw new Error("A private V6 ratings object exceeds the expected size.");
    }
    chunks.push(buffer);
  }
...
async function destinationFor(root, sessionId, digest) {
  ...
  if (
    ...
    throw new Error("Unsafe private V6 inbox destination.");
  }
  await mkdir(directory, { recursive: true, mode: 0o700 });
  const metadata = await lstat(directory);
  if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
    throw new Error("Private V6 inbox session path is not a safe directory.");
  }
  return destination;
}

export async function downloadV6(
  outputDirectory,
  ...
  do {
    ...
    if (
      ...
      throw new Error("Private V6 inbox listing returned an invalid page.");
    }
    for (const blob of page.blobs) {
      ...
      if (
        ...
        blob.size < 2 ||
        blob.size > MAX_V6_STORED_BYTES
      ) {
        throw new Error("Unexpected object in the private V6 inbox.");
      }
      ...
      if (
        ...
        throw new Error("A private V6 ratings object could not be downloaded.");
      }
      const bytes = await readBounded(result.stream);
      if (bytes.length !== blob.size) {
        throw new Error("A private V6 ratings object changed during download.");
      }
      parseV6StoredRecordBytes(bytes, blob.pathname);
      ...
      try {
        ...
        if (!local.equals(bytes)) {
          throw new Error("An existing V6 inbox file failed integrity comparison.");
        }
        existing += 1;
      }
    }
    if (page.hasMore && typeof page.cursor !== "string") {
      throw new Error("Private V6 inbox listing omitted its continuation cursor.");
    }
...
async function main() {
  ...
  if (!acknowledged || positional.length !== 1) {
    console.error(
      "Usage: npm run ratings:v6-inbox -- <output-dir> --acknowledge-private-data",
    );
    process.exitCode = 2;
    return;
  }
  try {
    const result = await downloadV6(positional[0]);
    console.log(
      `Private V6 inbox sync complete: ${result.downloaded} downloaded, ${result.existing} already present.`,
    );
  } catch {
    console.error(
      "Private V6 inbox download failed. Check authorized Blob credentials and retry.",
    );
    process.exitCode = 1;
  }
}
