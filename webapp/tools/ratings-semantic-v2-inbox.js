...
async function readBounded(stream) {
  ...
  for await (const chunk of stream) {
    ...
    size += buffer.length;
    if (size > MAX_SEMANTIC_STORED_BYTES) {
      throw new Error(
        "A private semantic ratings object exceeds the expected size.",
      );
    }
    chunks.push(buffer);
  }
...
async function destinationFor(root, sessionId, digest) {
  ...
  if (
    ...
    throw new Error("Unsafe private semantic inbox destination.");
  }
  await mkdir(directory, { recursive: true, mode: 0o700 });
  const metadata = await lstat(directory);
  if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
    throw new Error(
      "Private semantic inbox session path is not a safe directory.",
    );
  }
  return destination;
}

export async function downloadSemantic(
  outputDirectory,
  ...
  do {
    const page = await storage.list({
      prefix: SEMANTIC_BLOB_PREFIX,
      limit: 1000,
      cursor,
    });
    if (!page || !Array.isArray(page.blobs) || typeof page.hasMore !== "boolean") {
      throw new Error("Private semantic inbox listing returned an invalid page.");
    }
    for (const blob of page.blobs) {
      const match = PATHNAME.exec(blob.pathname);
      if (!match) {
        throw new Error("Unexpected object path in the private semantic inbox.");
      }
      if (
        !Number.isInteger(blob.size) ||
        blob.size < 2 ||
        blob.size > MAX_SEMANTIC_STORED_BYTES
      ) {
        throw new Error(
          "A private semantic ratings object has an invalid listed size.",
        );
      }
      ...
      if (
        ...
        throw new Error(
          "A private semantic ratings object could not be downloaded.",
        );
      }
      const bytes = await readBounded(result.stream);
      if (bytes.length !== blob.size) {
        throw new Error(
          "A private semantic ratings object size changed during download.",
        );
      }
      parseSemanticStoredRecordBytes(bytes, blob.pathname);
      ...
      try {
        ...
        if (!local.equals(bytes)) {
          throw new Error(
            "An existing private semantic inbox file failed integrity comparison.",
          );
        }
        existing += 1;
      }
    ...
    if (page.hasMore && typeof page.cursor !== "string") {
      throw new Error(
        "Private semantic inbox listing omitted its continuation cursor.",
      );
    }
...
async function main() {
  ...
  try {
    const result = await downloadSemantic(positional[0]);
    console.log(
      `Private semantic inbox sync complete: ${result.downloaded} downloaded, ${result.existing} already present.`,
    );
  } catch {
    console.error(
      "Private semantic inbox download failed. Check authorized Blob credentials and retry.",
    );
    process.exitCode = 1;
  }
}
