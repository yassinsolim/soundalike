...
export const MAX_V2_BODY_BYTES = 256 * 1024;
export const MAX_V2_STORED_BYTES = 300 * 1024;
export const V2_PROTOCOL_SHA256 =
  "1f7a3cc48ecb62f85d3c3d65fa0f0c0fa5cd73eeccd67049d7bc5e84d1dcc227";
export const V2_PILOT_PACK_SHA256 =
  "1980da60810959e7cdd24f39bd7142c8e34c76dab633c705976b85e49b297023";
export const V2_BLOB_PREFIX = "human-ratings/fulltrack-v2/";

const SUBMISSION_SCHEMA = "fulltrack_jamendo_listener_submission_v2";
const PROVIDER = "hosted_private_fulltrack_v2_evaluator";
...
const RATER_ID = /^anon-v2-[a-f0-9]{24}$/;
const SESSION_ID = /^fulltrack-session-[a-f0-9]{24}$/;
const LIST_ID = /^list-[a-f0-9]{24}$/;
...
function buildCommittedListIds() {
  if (
    ...
    protocol.protocol_kind !==
      "fulltrack_jamendo_blind_listener_v2_private_submission" ||
    protocol.submission_schema !== SUBMISSION_SCHEMA ||
    protocol.content_sha256 !== V2_PROTOCOL_SHA256 ||
    protocol.pilot_pack_sha256 !== V2_PILOT_PACK_SHA256 ||
    protocol.submission_endpoint !== "/api/ratings-v2" ||
    protocol.private_blob_prefix !== V2_BLOB_PREFIX ||
    ...
    documentHash(protocol) !== V2_PROTOCOL_SHA256 ||
    pilotPack.schema_version !== 2 ||
    pilotPack.pack_kind !== "fulltrack_jamendo_blind_pilot_v2" ||
    pilotPack.pack_id !== "fulltrack-v2-pilot-20" ||
    pilotPack.rankings_state !== "LOCKED_BEFORE_RATINGS" ||
    pilotPack.ratings_count_at_freeze !== 0 ||
    pilotPack.seed_count !== 20 ||
    pilotPack.method_count !== 4 ||
    pilotPack.results_per_method !== 5 ||
    pilotPack.fold !== 0 ||
    pilotPack.fold_part !== "test" ||
    pilotPack.research_only !== true ||
    pilotPack.promotion_allowed !== false ||
    pilotPack.content_sha256 !== V2_PILOT_PACK_SHA256 ||
    documentHash(pilotPack) !== V2_PILOT_PACK_SHA256 ||
    !Array.isArray(pilotPack.seeds) ||
    pilotPack.seeds.length !== 20
  ) {
    throw new Error("Committed v2 ratings protocol is inconsistent");
  }
  const listIds = new Set();
  for (const seed of pilotPack.seeds) {
    if (!Array.isArray(seed.lists) || seed.lists.length !== 4) {
      throw new Error("Committed v2 list cardinality is inconsistent");
    }
    for (const list of seed.lists) {
      if (
        ...
        throw new Error("Committed v2 list identity is inconsistent");
      }
      list.ranking.forEach((row, index) => {
        if (row.position !== index + 1) {
          throw new Error("Committed v2 list ranking is inconsistent");
        }
      });
  ...
  if (listIds.size !== 80) {
    throw new Error("Committed v2 list count is inconsistent");
  }
  const forbidden = [
  ...
  if (forbidden.some((marker) => canonical(pilotPack).includes(marker))) {
    throw new Error("Committed v2 pilot is not blinded");
  }
  return listIds;
}
...
function validateEvidence(ratings, requireRating = true) {
  ...
  if (
    ...
    ratings.protocol_sha256 !== V2_PROTOCOL_SHA256 ||
    ratings.pilot_pack_sha256 !== V2_PILOT_PACK_SHA256 ||
    startedAt === null ||
    lastActivityAt === null ||
    exportedAt === null ||
    ...
    Object.keys(ratings.list_ratings).length > 80
  ) {
    return null;
  }
  let count = 0;
...
export function validateV2Export(ratings) {
  if (
...
export function validateV2StoredRecord(document, pathname) {
  ...
  const expectedPath =
    `${V2_BLOB_PREFIX}${ratings.session_id}/${digest}.json`;
  if (
...
export function parseV2StoredRecordBytes(value, pathname) {
  const bytes = Buffer.isBuffer(value) ? value : Buffer.from(value);
  if (bytes.length < 2 || bytes.length > MAX_V2_STORED_BYTES) {
    throw new Error("Invalid private v2 ratings record size");
  }
  const text = new TextDecoder("utf-8", { fatal: true }).decode(bytes);
  const document = strictJsonParse(text);
  const validated = validateV2StoredRecord(document, pathname);
  if (!validated || text !== `${canonical(document)}\n`) {
    throw new Error("Invalid private v2 ratings record");
  }
...
async function readBody(request) {
  ...
  if (
    length !== undefined &&
    (!/^(0|[1-9]\d*)$/.test(length) || Number(length) > MAX_V2_BODY_BYTES)
  ) {
  ...
  if (raw.length > MAX_V2_BODY_BYTES) {
    const error = new Error("payload");
    error.statusCode = 413;
    throw error;
  }
...
export function createV2Handler(
  ...
  return async function ratingsV2Handler(request, response) {
    ...
    if (
      ...
      wrapper.study !== "fulltrack-v2-pilot"
    ) {
      return send(response, 400, { error: "invalid request" });
    }
    let accepted;
    try {
      accepted = validateV2Export(wrapper.ratings);
    } catch {
      accepted = null;
    }
    if (!accepted) {
    ...
    const pathname =
      `${V2_BLOB_PREFIX}${sanitized.session_id}/${receiptHash}.json`;
    if (!validateV2StoredRecord(stored, pathname)) {
...
export default createV2Handler();
